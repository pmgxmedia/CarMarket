...
const SellerMarketplace = () => {
  ...
  const fetchAds = async () => {
    ...
    try {
      ...
      params.append('excludeMyAds', 'true')
    ...
    <div className="space-y-4 md:space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button onClick={() => navigate('/seller/dashboard')} className="text-gray-500 hover:text-gray-700">
            ← Back
          </button>
          <h1 className="text-xl md:text-2xl font-bold text-gray-800 dark:text-white">Marketplace</h1>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            {ads.map((ad) => (
              <Link key={ad._id} to={`/seller/marketplace/${ad._id}`} className="group">
          ...
          {pagination && pagination.pages > 1 && (
            <div className="flex items-center justify-center gap-2">
              <button
                ...
                disabled={page === 1}
                className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm md:text-base"
              >
                Previous
              </button>
              <span className="px-4 py-2 text-gray-700 dark:text-gray-200 text-sm md:text-base">
